...
class TriggerSequence:
    ...
    def update_galvo_scan_parameters(self, sws=None):
        original_values = {"galvo_sw_states": self.galvo_sw_states}
        try:
            if sws is not None:
    ...
    def generate_digital_triggers(self, lasers, camera, slm_seq):
        ...
        if slm_seq == "None":
            ...
            switch_trigger[self.digital_starts[cam_ind] - self.galvo_sw_settle_samples:self.digital_ends[cam_ind] + 1] = self.galvo_sw_states[camera]
        else:
